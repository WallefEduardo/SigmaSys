...
import { ConsumableUnit, isAreaBasedUnit, convertToBaseUnit } from "./consumable-units";
...
export function calculatePrintJobCost(
  ...
  if (!passConfig) {
    throw new Error(`Passada "${job.selectedPass}" não encontrada no equipamento`);
  }
  ...
  if (passConfig.selectedInks) {
    for (const inkId of passConfig.selectedInks) {
      ...
      if (ink && isAreaBasedUnit(ink.unit)) {
        ...
        const consumption = calculateInkConsumption(totalArea, ink.unit, passConfig.inkConsumption);
...
export function calculateQuickEstimate(
  ...
  averageSpeed: number = 60, // m²/h
  averageInkCost: number = 50 // R$ por m²
): number {
