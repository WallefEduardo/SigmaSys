 ...
 */
export const createOptimisticMutation = <TData, TVariables extends { id?: string }>(
  ...
  return {
    ...
    onMutate: async (variables: TVariables) => {
      ...
      if (options.type === 'create') {
        queryUtils.optimisticAdd(options.queryClient, options.queryKey, variables);
      } else if (options.type === 'update' && variables.id) {
        queryUtils.optimisticUpdate(options.queryClient, options.queryKey, variables as any);
      } else if (options.type === 'delete' && variables.id) {
        queryUtils.optimisticRemove(options.queryClient, options.queryKey, variables.id);
      }
    ...
    onError: (error: Error, variables: TVariables, context: any) => {
      ...
      if (context?.previousData) {
        queryUtils.rollback(options.queryClient, options.queryKey, context.previousData);
      }
