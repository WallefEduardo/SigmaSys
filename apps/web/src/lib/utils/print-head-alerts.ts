 ...
 */
export function calculateLifePercentage(currentUse: number, lifespan: number): number {
 ...
 */
export function generatePrintHeadAlerts(printHead: PrintHeadStatus): PrintHeadAlert[] {
  const alerts: PrintHeadAlert[] = [];
  const lifePercentage = calculateLifePercentage(printHead.currentUse, printHead.lifespan);
  const averageUsage = calculateAverageUsagePerDay(printHead.currentUse, printHead.installationDate);
  const daysRemaining = estimateDaysRemaining(printHead.currentUse, printHead.lifespan, averageUsage);
  ...
  // Alerta de manutenção preventiva - entre 50% e 80%
    ...
    alerts.push({
      ...
      message: `A ${printHead.name} (${printHead.model}) está com ${lifePercentage.toFixed(1)}% da vida útil consumida.`,
      actionRequired: "Considere realizar limpeza e verificação da qualidade de impressão",
      estimatedDaysRemaining: daysRemaining,
      createdAt: new Date(),
      acknowledged: false
    });
  ...
  const daysSinceInstallation = Math.floor((new Date().getTime() - printHead.installationDate.getTime()) / (1000 * 60 * 60 * 24));
  if (daysSinceInstallation > 365) {
    alerts.push({
      ...
      actionRequired: "Monitore a qualidade de impressão e considere substituição preventiva",
      createdAt: new Date(),
      acknowledged: false
    });
  }
 ...
 */
export function getAllPrintHeadAlerts(printHeads: PrintHeadStatus[]): PrintHeadAlert[] {
 ...
 */
export function getAlertsByPriority(alerts: PrintHeadAlert[], priority: PrintHeadAlert["priority"]): PrintHeadAlert[] {
 ...
 */
export function getUnacknowledgedAlerts(alerts: PrintHeadAlert[]): PrintHeadAlert[] {
  if (!alerts || !Array.isArray(alerts)) {
    return [];
  }
