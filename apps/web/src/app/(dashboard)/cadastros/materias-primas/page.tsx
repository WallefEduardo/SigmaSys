...
export default function MateriasPage() {
	...
	const createMutation = api.materials.create.useMutation({
		...
		onError: (error) => {
			...
			if (
				...
				errorMessage =
					"Já existe um material com este código. Use um código diferente ou deixe em branco para gerar automaticamente.";
			}
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		if (!formData.cost || isNaN(Number.parseFloat(formData.cost))) {
		...
		const dataToSend = {
			...
			minStock: formData.minStock
				? Number.parseFloat(formData.minStock)
				: undefined,
		};
						...
						<form onSubmit={handleSubmit} className="space-y-4">
							...
							<div>
								...
								<p className="mt-1 text-muted-foreground text-xs">
									Deixe em branco para gerar automaticamente (MAT-001,
									MAT-002...)
								</p>
							</div>
					...
					<Card key={material.id}>
						...
						<CardContent>
							<div className="space-y-2 text-sm">
								...
								{material.minStock && (
									<div>
										<strong>Estoque Mín.:</strong>{" "}
										{Number(material.minStock).toFixed(2)}
									</div>
								)}
							</div>
						</CardContent>
