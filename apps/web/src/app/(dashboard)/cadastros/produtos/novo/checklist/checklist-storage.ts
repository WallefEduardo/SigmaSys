...
class ChecklistStorage {
	 ...
	 */
	static save(
		config: SavedChecklistData["config"],
		productId?: string,
	): boolean {
		try {
			const dataToSave: SavedChecklistData = {
				version: ChecklistStorage.VERSION,
				timestamp: Date.now(),
				productId,
				config: ChecklistStorage.sanitizeConfig(config),
			};

			// Compressão básica: remover propriedades desnecessárias
			const compressedData = ChecklistStorage.compressData(dataToSave);

			localStorage.setItem(
				ChecklistStorage.STORAGE_KEY,
	...
	static load(): SavedChecklistData | null {
		try {
			const stored = localStorage.getItem(ChecklistStorage.STORAGE_KEY);
			if (!stored) return null;

			const data: SavedChecklistData = JSON.parse(stored);

			// Validar estrutura dos dados
			if (!ChecklistStorage.validateData(data)) {
				console.warn(
					"⚠️ CHECKLIST-STORAGE - Dados inválidos encontrados, limpando...",
				);
			...
			if (daysOld > ChecklistStorage.MAX_STORAGE_AGE_DAYS) {
				ChecklistStorage.clear();
				return null;
			}
	...
	static clear(): void {
		try {
			localStorage.removeItem(ChecklistStorage.STORAGE_KEY);
		} catch (error) {
	...
	static getLastModified(): Date | null {
		try {
			const data = ChecklistStorage.load();
	...
	static hasChanges(currentConfig: SavedChecklistData["config"]): boolean {
		try {
			const saved = ChecklistStorage.load();
			if (!saved) return true;

			const currentSerialized = JSON.stringify(
				ChecklistStorage.sanitizeConfig(currentConfig),
			);
			const savedSerialized = JSON.stringify(
				ChecklistStorage.sanitizeConfig(saved.config),
			);
		...
		return {
			...
			config: {
				...
				nodes: data.config.nodes.map((node) => ({
					...node,
					data: ChecklistStorage.removeCallbacks(node.data),
				})),
			},
		};
	}
	...
	static getStats(): {
		...
		try {
			const data = ChecklistStorage.load();
			const rawData = localStorage.getItem(ChecklistStorage.STORAGE_KEY);
