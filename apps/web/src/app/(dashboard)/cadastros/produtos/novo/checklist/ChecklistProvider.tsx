...
function checklistReducer(
	...
	switch (action.type) {
		case "LOAD_DATA": {
			...
			const validatedNodes = action.payload.nodes.map(
				(node: any, index: number) => {
					...
					if (!hasValidPosition) {
						console.warn(
							"🚨 CHECKLISTPROVIDER - Node sem position válida detectado:",
							node.id,
							"position:",
							node.position,
						);
		...
		case "UPDATE_NODES": {
			...
			const validatedUpdateNodes = action.payload.map(
				(node: any, index: number) => ({
					...node,
					position:
						node.position &&
		...
		case "ADD_NODE": {
			...
			const nodeToAdd = {
				...
				position:
					...
					typeof action.payload.position.x === "number" &&
					typeof action.payload.position.y === "number"
						? action.payload.position
