...
import { z } from "zod";
import { ArrowLeft, Save, Trash2, Loader2, HelpCircle, Database } from "lucide-react";
...
const consumableSchema = z.object({
  ...
  unit: z.string().min(1, "Unidade é obrigatória").refine((val) => 
    Object.values(ConsumableUnit).includes(val as ConsumableUnit), 
...
export default function EditConsumablePage() {
  ...
  const { data: consumable, isLoading, error } = api.consumables.byId.useQuery({
    id: consumableId,
  });
  ...
  const form = useForm<ConsumableFormData>({
    ...
    defaultValues: React.useMemo(() => {
      ...
      console.log("🔄 Criando defaultValues com:", { type: consumable.type, unit: consumable.unit });
  ...
  if (error) {
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center">
            <div className="text-destructive mb-4">Erro ao carregar insumo</div>
            <p className="text-muted-foreground mb-4">{error.message}</p>
            <Button onClick={() => router.push("/cadastros/insumos")} variant="outline">
              Voltar para lista
            </Button>
          </div>
        </div>
  ...
  if (!consumable) {
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center">
            <div className="text-muted-foreground mb-4">Insumo não encontrado</div>
            <Button onClick={() => router.push("/cadastros/insumos")} variant="outline">
              Voltar para lista
            </Button>
          </div>
    ...
    <TooltipProvider>
      ...
      <div className="flex items-center gap-4">
        ...
        <Button 
          ...
          className="ml-auto"
          disabled={updateConsumableMutation.isPending || deleteConsumableMutation.isPending}
        >
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{watchedType === "ink" ? "Custo por Litro *" : "Custo *"}</FormLabel>
                      <FormControl>
                        <CurrencyInput 
                          value={field.value}
                ...
                {watchedType !== "ink" && (
                  ...
                  render={({ field }) => {
                    ...
                    const currentType = watchedType || consumable?.type || "other";
                    
                    let recommendedUnits = currentType && currentType !== "other"
                    ...
                    if (field.value && !recommendedUnits.includes(field.value as ConsumableUnit)) {
                      recommendedUnits = [...recommendedUnits, field.value as ConsumableUnit];
                      console.log("🔧 Adicionando unidade atual às opções:", field.value);
                    }
                                ...
                                <Tooltip>
                                  ...
                                  <TooltipContent className="max-w-xs">
                                    <div className="space-y-2 text-xs">
                                      ...
                                      <p>Tempo estimado de vida útil da cabeça baseado na experiência operacional</p>
                                    </div>
                                  </TooltipContent>
                                </Tooltip>
                              </FormLabel>
