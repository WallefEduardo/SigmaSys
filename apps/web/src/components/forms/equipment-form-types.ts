...
export const equipmentFormSchema = z.object({
	...
	energyCostPerHour: z
		.number()
		.min(0, "Custo de energia deve ser positivo")
		.max(999999, "Custo muito alto (máx: R$ 999.999)")
		.optional(),
	maintenanceCostPerHour: z
		.number()
		.min(0, "Custo de manutenção deve ser positivo")
		.max(999999, "Custo muito alto (máx: R$ 999.999)")
		.optional(),
	...
	acquisitionValue: z
		.number()
		.min(0, "Valor de aquisição deve ser positivo")
		.max(9999999, "Valor muito alto (máx: R$ 9.999.999)")
		.optional(),
	residualValue: z
		.number()
		.min(0, "Valor residual deve ser positivo")
		.max(9999999, "Valor muito alto (máx: R$ 9.999.999)")
		.optional(),
	depreciationMethod: z.enum(["linear", "accelerated"]).optional(),
	usefulLifeHours: z
		.number()
		.min(1, "Vida útil deve ser pelo menos 1 hora")
		.optional(),
	usefulLifeYears: z
		.number()
				...
				inkConsumables: z
					.array(
						z.object({
							...
							consumptionMlPerM2: z
								.number()
								.min(0, "Consumo deve ser positivo"), // ml por m²
						}),
					)
					.default([]),
	...
	maintenanceInterval: z
		...
		.max(3650, "Intervalo máximo de 10 anos")
		.optional(),
...
export const validateDepreciation = (data: Partial<EquipmentFormData>) => {
	...
	if (data.usefulLifeHours && data.usefulLifeYears) {
		...
		if (diff > 0.5) {
			errors.push(
				"Vida útil em horas e anos deve ser consistente (aprox. 2000h/ano)",
			);
		}
	}
...
export const getDefaultPassConfigurations = (): Record<
	string,
	PassConfiguration
> => ({
	draft: {
