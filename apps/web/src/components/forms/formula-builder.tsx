...
import { Calculator, Check, X, Info, Plus } from 'lucide-react'
...
const availableVariables = [
  { name: 'largura', description: 'Largura do produto em metros', example: '2.5' },
  { name: 'altura', description: 'Altura do produto em metros', example: '1.8' },
  ...
  { name: 'perimetro', description: 'Perímetro calculado: 2 * (largura + altura)', example: '8.6' },
  { name: 'area', description: 'Área calculada: largura * altura', example: '4.5' },
  { name: 'volume', description: 'Volume calculado: largura * altura * espessura', example: '13.5' },
]

// Funções matemáticas disponíveis
const availableFunctions = [
  { name: 'max', description: 'Valor máximo entre dois números', example: 'max(largura, 2)' },
  { name: 'min', description: 'Valor mínimo entre dois números', example: 'min(altura, 3)' },
  { name: 'round', description: 'Arredondamento', example: 'round(area, 2)' },
  { name: 'ceil', description: 'Arredondamento para cima', example: 'ceil(largura)' },
  { name: 'floor', description: 'Arredondamento para baixo', example: 'floor(altura)' },
...
export function FormulaBuilder({ value, onChange, targetUnit, onTargetUnitChange }: FormulaBuilderProps) {
  ...
  const calculatedContext = {
    ...
    volume: previewContext.largura * previewContext.altura * (previewContext.espessura / 1000), // espessura em metros
  }

  // Simular validação de fórmula (em produção seria via API)
  const validateFormula = (formula: string, context: Record<string, number>): ValidationResult => {
    if (!formula.trim()) {
          ...
          <div className="space-y-2">
            ...
            {onTargetUnitChange && (
              <div className="flex items-center gap-2">
                <label className="text-sm font-medium">Unidade do resultado:</label>
                ...
                <Tooltip>
                  ...
                  <TooltipContent side="right">
                    <div className="max-w-xs">
                      ...
                      <div className="text-xs mt-1">Exemplo: {variable.example}</div>
                    </div>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
                ...
                <Tooltip>
                  ...
                  <TooltipContent side="right">
                    <div className="max-w-xs">
                      ...
                      <div className="text-xs mt-1 font-mono">{func.example}</div>
                    </div>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="bg-muted/50 p-3 rounded">
            <div className="text-sm font-medium mb-2">Variáveis Calculadas:</div>
          ...
          {validation.valid && validation.result !== undefined && (
            <div className="bg-green-50 dark:bg-green-950/20 p-4 rounded border border-green-200 dark:border-green-800">
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium text-green-600">Resultado da Fórmula:</span>
