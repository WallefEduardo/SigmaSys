  ...
  const calculateFormula = React.useCallback((
    ...
    try {
      ...
      const extendedContext = {
        ...variables,
        perimetro: variables.largura && variables.altura 
          ? 2 * (variables.largura + variables.altura) 
          : 0,
        area: variables.largura && variables.altura 
          ? variables.largura * variables.altura 
          : 0,
        volume: variables.largura && variables.altura && variables.espessura
          ? variables.largura * variables.altura * (variables.espessura / 1000)
          : 0
      }

      // Substituir variáveis na fórmula
      let processedFormula = formulaText
      const usedVariables: Array<{ name: string, value: number, unit?: string }> = []
      ...
      const calculatedValue = Function(`"use strict"; return (${processedFormula})`)()
      ...
      return {
        ...
        breakdown: generateBreakdown(formulaText, extendedContext, calculatedValue)
      }
    } catch (err) {
      return null
    }
      ...
      <Card className={className}>
        <CardContent className="p-4">
          <div className="flex items-center gap-2 text-muted-foreground">
            ...
            <span className="text-sm">Digite uma fórmula para ver o resultado</span>
          </div>
        </CardContent>
      </Card>
    )
