...
import { Info, Calculator, TrendingDown, AlertTriangle } from "lucide-react";
import { EquipmentFormData, validateDepreciation } from "../equipment-form-types";
...
export function DepreciationTab({ form }: DepreciationTabProps) {
  ...
  const calculateDepreciation = React.useMemo(() => {
    ...
    const depreciableValue = watchedValues.acquisitionValue - watchedValues.residualValue;
    ...
    return {
      ...
      annualDepreciation,
      hourlyDepreciation,
      depreciationRate: watchedValues.acquisitionValue > 0 
  ...
  const depreciationMethods = [
    { value: "linear", label: "Linear (Uniforme)", description: "Depreciação constante ao longo do tempo" },
    { value: "accelerated", label: "Acelerada", description: "Maior depreciação nos primeiros anos" },
  ];
    ...
    <div className="space-y-6">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              {errors.residualValue && (
                <p className="text-sm text-destructive">{errors.residualValue.message}</p>
              )}
              <p className="text-xs text-muted-foreground">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => setValue("depreciationMethod", value as "linear" | "accelerated")}
              >
                <SelectTrigger>
              ...
              {errors.depreciationMethod && (
                <p className="text-sm text-destructive">{errors.depreciationMethod.message}</p>
              )}
            </div>
          </CardContent>
        </Card>
            ...
            {watchedValues.usefulLifeHours && watchedValues.usefulLifeYears && (
              <div className="p-3 bg-muted rounded-md">
                ...
                <div className="text-sm text-muted-foreground mt-1 flex items-center gap-2">
                  <span>
                    {Math.round(watchedValues.usefulLifeHours / watchedValues.usefulLifeYears)} 
                    {" horas/ano "}
                  </span>
                  <Badge variant={
                    Math.abs((watchedValues.usefulLifeHours / watchedValues.usefulLifeYears) - 2000) < 500 
                      ? "default" 
                      : "secondary"
                  }>
                    {Math.abs((watchedValues.usefulLifeHours / watchedValues.usefulLifeYears) - 2000) < 500 
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-3">
                <div className="flex justify-between">
                  <span className="text-sm font-medium">Valor Depreciável:</span>
                ...
                <div className="flex justify-between">
                  <span className="text-sm font-medium">Taxa de Depreciação:</span>
                  <span className="text-sm">
              ...
              <div className="space-y-3">
                {calculateDepreciation.annualDepreciation > 0 && (
                  <div className="flex justify-between">
                    <span className="text-sm font-medium">Depreciação Anual:</span>
                    <span className="text-sm">
                      {new Intl.NumberFormat('pt-BR', {
                        style: 'currency',
                        currency: 'BRL'
                ...
                {calculateDepreciation.hourlyDepreciation > 0 && (
                  <div className="flex justify-between">
                    <span className="text-sm font-medium">Depreciação/Hora:</span>
            ...
            {calculateDepreciation.hourlyDepreciation > 0 && (
              <div className="mt-4 p-3 bg-white dark:bg-gray-800 rounded border">
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Este valor será automaticamente adicionado ao custo operacional do equipamento
                </p>
              </div>
            )}
          </CardContent>
      ...
      <Card className="border-blue-200 bg-blue-50 dark:bg-blue-950/20">
        ...
        <CardContent className="text-sm text-blue-700 dark:text-blue-400 space-y-2">
          <p>
            <strong>Método Linear:</strong> A depreciação é uniforme durante toda a vida útil. 
            Ideal para equipamentos com uso constante.
          </p>
          <p>
            <strong>Método Acelerado:</strong> Maior depreciação nos primeiros anos (60% nos primeiros 30% da vida útil). 
            Adequado para equipamentos com alta obsolescência tecnológica.
          </p>
          <p>
            <strong>Vida Útil Recomendada:</strong> Impressoras industriais: 8-12 anos (16.000-24.000h). 
            Equipamentos de usinagem: 15-20 anos (30.000-40.000h).
          </p>
        </CardContent>
      </Card>
    </div>
