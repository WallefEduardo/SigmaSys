...
export function ReportsTab({ equipmentId, equipmentType }: ReportsTabProps) {
	...
	const profitMargin =
		((mockData.totalRevenue - mockData.totalCosts) / mockData.totalRevenue) *
		100;
		...
		<div className="space-y-6">
			...
			<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
				...
				<Card>
					...
					<CardContent>
						<div className="font-bold text-2xl">{profitMargin.toFixed(1)}%</div>
						<p className="text-muted-foreground text-xs">
							{new Intl.NumberFormat("pt-BR", {
								style: "currency",
								currency: "BRL",
							}).format(mockData.totalRevenue - mockData.totalCosts)}{" "}
							lucro
						</p>
					</CardContent>
				</Card>
				...
				<Card>
					...
					<CardContent>
						<div className="font-bold text-2xl">
							{mockData.totalOperatingHours}h
						</div>
						<p className="text-muted-foreground text-xs">
				...
				<Card>
					<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
						<CardTitle className="font-medium text-sm">Eficiência</CardTitle>
						<Gauge className="h-4 w-4 text-muted-foreground" />
					</CardHeader>
					<CardContent>
						<div className="font-bold text-2xl">{mockData.efficiency}%</div>
						<p className="text-muted-foreground text-xs">
							<Badge
								variant={mockData.efficiency > 85 ? "default" : "secondary"}
							>
					...
					<CardContent className="space-y-4">
						<div className="space-y-3">
							<div className="flex justify-between">
								<span className="font-medium text-sm">Custo por Hora:</span>
							...
							<div className="flex justify-between">
								<span className="font-medium text-sm">Receita por Hora:</span>
								<span className="text-sm">
									{new Intl.NumberFormat("pt-BR", {
										style: "currency",
										currency: "BRL",
									}).format(revenuePerHour)}
								</span>
							</div>
							...
							<div className="flex justify-between">
								...
								<span className="text-sm">
									{new Intl.NumberFormat("pt-BR", {
										style: "currency",
										currency: "BRL",
									}).format(mockData.currentDepreciation)}
								</span>
							</div>
						</div>
						...
						<div>
							<h5 className="mb-2 font-medium text-sm">
								Consumíveis por Tipo:
							</h5>
							<div className="space-y-2">
								{Object.entries(mockData.consumablesUsed).map(
									([type, data]) => (
					...
					<CardContent className="space-y-4">
						<div>
							<div className="mb-2 flex items-center justify-between">
								<span className="font-medium text-sm">Taxa de Utilização</span>
						...
						<div className="grid grid-cols-2 gap-4 text-sm">
							...
							<div className="text-center">
								<div className="font-semibold text-lg">
									{(mockData.totalOperatingHours / mockData.totalJobs).toFixed(
										1,
									)}
									h
								</div>
					...
					<CardContent>
						<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
							{Object.entries(mockData.passesUsage).map(([passType, data]) => (
								<div key={passType} className="rounded-lg border p-4">
									<div className="mb-2 flex items-center justify-between">
										<h5 className="font-medium capitalize">{passType}</h5>
										<Badge
											variant={passType === "normal" ? "default" : "secondary"}
										>
											{data.count} jobs
										</Badge>
									</div>
										...
										<div className="flex justify-between">
											...
											<span>
												{new Intl.NumberFormat("pt-BR", {
													style: "currency",
													currency: "BRL",
												}).format(data.revenue / data.hours)}
											</span>
										</div>
									</div>
								</div>
							...
							<div key={month.month} className="space-y-2">
								<div className="flex items-center justify-between">
									...
									<div className="flex items-center gap-4 text-sm">
										...
										<Badge
											variant={
												(month.revenue - month.costs) / month.revenue > 0.6
													? "default"
													: "secondary"
											}
										>
											{(
												((month.revenue - month.costs) / month.revenue) *
												100
											).toFixed(1)}
											%
										</Badge>
									...
									<div className="flex items-center gap-2">
										...
										<Progress
											value={
												(month.revenue /
													Math.max(
														...mockData.monthlyStats.map((m) => m.revenue),
													)) *
												100
											}
									...
									<div className="flex items-center gap-2">
										...
										<Progress
											value={
												(month.costs /
													Math.max(
														...mockData.monthlyStats.map((m) => m.costs),
													)) *
												100
											}
											className="h-2 flex-1"
			...
			<Card className="border-yellow-200 bg-yellow-50 dark:bg-yellow-950/20">
				...
				<CardContent className="space-y-2 text-sm text-yellow-700 dark:text-yellow-400">
					{mockData.efficiency < 80 && (
						<p>
							• <strong>Eficiência baixa:</strong> Revisar configurações de
							passadas e manutenção preventiva.
						</p>
					)}
					{utilizationRate < 60 && (
						<p>
							• <strong>Baixa utilização:</strong> Equipamento subutilizado.
							Considerar mais projetos ou redistribuição de carga.
						</p>
					)}
					{profitMargin < 50 && (
						<p>
							• <strong>Margem baixa:</strong> Revisar precificação ou otimizar
							custos operacionais.
						</p>
					)}
					<p>
						• <strong>Performance geral:</strong>{" "}
						{mockData.efficiency > 85 &&
						profitMargin > 60 &&
						utilizationRate > 70
