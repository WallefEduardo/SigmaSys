...
export function MaintenanceTab({ form }: MaintenanceTabProps) {
	...
	const calculateNextMaintenance = React.useMemo(() => {
		...
		const nextDate = new Date(
			today.getTime() + watchedValues.maintenanceInterval * 24 * 60 * 60 * 1000,
		);
	...
	const removeTag = (tagIndex: number) => {
		const updatedTags = watchedValues.tags.filter(
	...
	const removeDocument = (docIndex: number) => {
		const updatedDocs = watchedValues.documents.filter(
	...
	const removeImage = (imgIndex: number) => {
		const updatedImages = watchedValues.images.filter(
		...
		<div className="space-y-6">
				...
				>
					<CardHeader className="pb-3">
						...
						>
							...
							Próxima Manutenção
							<Badge
								className={getMaintenanceUrgencyColor(
									calculateNextMaintenance.urgency,
								)}
							>
								{calculateNextMaintenance.urgency === "urgent"
									? "Urgente"
					...
					<CardContent className="grid gap-4 md:grid-cols-2">
						<div>
							<div className="font-medium text-sm">Data prevista:</div>
							<div className="text-lg">
								{calculateNextMaintenance.date.toLocaleDateString("pt-BR")}
							</div>
						</div>
						<div>
							<div className="font-medium text-sm">Tempo restante:</div>
							<div className="text-lg">
							...
							<div className="rounded-lg border p-3">
								<div className="flex items-center justify-between">
									<div className="flex items-center gap-2">
										...
										<span className="font-medium text-sm">Manual Oficial</span>
									</div>
									<Button variant="outline" size="sm" asChild>
										<a
											href={watchedValues.manualUrl}
											target="_blank"
							...
							<Button
								...
								onClick={() => {
									const input = document.querySelector(
										'input[placeholder="URL do documento"]',
									) as HTMLInputElement;
									if (input?.value) {
			...
			<Card>
				...
				<CardContent className="space-y-4">
					...
					<div>
						<h5 className="mb-2 font-medium text-sm">Tags Sugeridas</h5>
						<div className="flex flex-wrap gap-2">
							{suggestedTags
					...
					<div className="flex gap-2">
						...
						<Button
							...
							onClick={() => {
								const input = document.querySelector(
									'input[placeholder="Nova tag personalizada"]',
								) as HTMLInputElement;
								if (input?.value) {
									addTag(input.value);
									input.value = "";
			...
			<Card className="border-blue-200 bg-blue-50 dark:bg-blue-950/20">
				...
				<CardContent className="space-y-2 text-blue-700 text-sm dark:text-blue-400">
					<p>
						<strong>Impressoras:</strong> Limpeza semanal, calibração mensal,
						substituição de cabeças conforme desgaste.
					</p>
					<p>
						<strong>Equipamentos de Usinagem:</strong> Lubrificação regular,
						verificação de ferramentas, calibração de eixos.
					</p>
					<p>
						<strong>Documentação:</strong> Mantenha fotos e manuais acessíveis
						para equipe técnica.
					</p>
					<p>
						<strong>Intervalos:</strong> Equipamentos críticos: 30-60 dias.
						Equipamentos auxiliares: 90-180 dias.
					</p>
				</CardContent>
			</Card>
				...
				<Card className="border-red-200 bg-red-50 dark:bg-red-950/20">
					<CardContent className="pt-4">
						...
						<ul className="mt-2 text-red-700 text-sm dark:text-red-400">
							{errors.maintenanceInterval && (
								<li>• {errors.maintenanceInterval.message}</li>
							)}
							{errors.maintenanceNotes && (
								<li>• {errors.maintenanceNotes.message}</li>
							)}
