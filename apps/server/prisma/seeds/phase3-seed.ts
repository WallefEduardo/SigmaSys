...
export async function seedPhase3() {
  console.log('🌱 Seeding FASE 3 - Produtos e Fórmulas...')

  // Buscar empresa e usuário existentes
  const company = await prisma.company.findFirst({
    where: { name: 'Empresa Teste Ltda' }
  })

  if (!company) {
    throw new Error('Empresa não encontrada. Execute as seeds da FASE 2 primeiro.')
  }
  ...
  const finishes = await prisma.$transaction(async (tx) => {
    const finishData = [
      ...
      },
      {
        ...
        composition: {
          materials: [
            { materialId: materials[3].id, quantity: 4, unit: 'un' }
          ],
          processes: [
