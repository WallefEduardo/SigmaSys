	...
	if (equipment) {
		...
		try {
			const { EquipmentCostCalculator } = await import(
				"./src/lib/equipment-cost-calculator.js"
			);
			const calculator = new EquipmentCostCalculator(prisma);

			const totalCostData = await calculator.getCostForProductCalculation(
				equipment.id,
			);
			...
			console.log(
				"- Dados da passada padrão:",
				totalCostData.passCost
					? {
			...
			if (totalCostData.passCost) {
				console.log(
					"✅ SUCESSO! Custo total calculado: R$",
					totalCostData.passCost.totalCostPerM2.toFixed(4),
					"/m²",
				);
			} else {
