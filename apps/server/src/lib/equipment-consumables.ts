...
export class EquipmentConsumablesService {
  ...
  static calculateConsumption(
    ...
    const results = consumables.map(consumable => {
      const rate = consumptionRates.find(r => 
  ...
  static monitorStock(
    ...
    return consumables.map(consumable => {
      ...
      const estimatedDaysRemaining = dailyUsageRate > 0 
      ...
      else if (consumable.currentStock <= consumable.minStock * 0.5) severity = 'high';
      else if (consumable.currentStock <= consumable.minStock) severity = 'medium';
      ...
      return {
        ...
        severity,
        estimatedDaysRemaining: estimatedDaysRemaining === Infinity 
          ? 999 
          : estimatedDaysRemaining,
        suggestedOrderQuantity: Math.round(suggestedOrderQuantity)
      };
    }).filter(alert => alert.severity !== 'low' || alert.estimatedDaysRemaining <= 30);
  }
  ...
  static updateStock(
    ...
    return consumables.map(consumable => {
      ...
      if (used) {
        return {
          ...consumable,
          currentStock: Math.max(0, consumable.currentStock - used.quantityUsed)
        };
      }
      return consumable;
    });
  ...
  static calculateCostPerSquareMeter(
    ...
    const consumption = this.calculateConsumption(1, passQuality, consumables, consumptionRates);
  ...
  static getDefaultConsumptionRates(): ConsumptionRate[] {
    return [
      // Tinta Ciano
      { passQuality: 'draft', consumableId: 'ink-cyan', rate: 2.5, unit: 'ml/m²' },
      { passQuality: 'normal', consumableId: 'ink-cyan', rate: 4.0, unit: 'ml/m²' },
      { passQuality: 'high', consumableId: 'ink-cyan', rate: 6.0, unit: 'ml/m²' },
      { passQuality: 'photo', consumableId: 'ink-cyan', rate: 8.5, unit: 'ml/m²' },
      
      // Tinta Magenta
      { passQuality: 'draft', consumableId: 'ink-magenta', rate: 2.5, unit: 'ml/m²' },
      { passQuality: 'normal', consumableId: 'ink-magenta', rate: 4.0, unit: 'ml/m²' },
      { passQuality: 'high', consumableId: 'ink-magenta', rate: 6.0, unit: 'ml/m²' },
      { passQuality: 'photo', consumableId: 'ink-magenta', rate: 8.5, unit: 'ml/m²' },
      
      // Tinta Amarela
      { passQuality: 'draft', consumableId: 'ink-yellow', rate: 2.0, unit: 'ml/m²' },
      { passQuality: 'normal', consumableId: 'ink-yellow', rate: 3.2, unit: 'ml/m²' },
      { passQuality: 'high', consumableId: 'ink-yellow', rate: 4.8, unit: 'ml/m²' },
      { passQuality: 'photo', consumableId: 'ink-yellow', rate: 7.0, unit: 'ml/m²' },
      
      // Tinta Preta
      { passQuality: 'draft', consumableId: 'ink-black', rate: 3.0, unit: 'ml/m²' },
      { passQuality: 'normal', consumableId: 'ink-black', rate: 4.8, unit: 'ml/m²' },
      { passQuality: 'high', consumableId: 'ink-black', rate: 7.2, unit: 'ml/m²' },
      { passQuality: 'photo', consumableId: 'ink-black', rate: 10.0, unit: 'ml/m²' },
      
      // Cabeça de impressão (desgaste por m²)
      { passQuality: 'draft', consumableId: 'printhead-dx5', rate: 0.0005, unit: 'wear/m²' },
      { passQuality: 'normal', consumableId: 'printhead-dx5', rate: 0.001, unit: 'wear/m²' },
      { passQuality: 'high', consumableId: 'printhead-dx5', rate: 0.0018, unit: 'wear/m²' },
      { passQuality: 'photo', consumableId: 'printhead-dx5', rate: 0.0025, unit: 'wear/m²' }
    ];
  }
  ...
  static generateConsumablesReport(
    ...
    const periodUsage = usageHistory.filter(usage => 
      usage.date >= period.start && usage.date <= period.end
    );

    const totalArea = periodUsage.reduce((sum, usage) => sum + usage.area, 0);
    const totalCost = periodUsage.reduce((sum, usage) => 
      sum + usage.consumables.reduce((consumableSum, c) => consumableSum + c.cost, 0)
    , 0);
    ...
    const qualityUsage: Record<string, { area: number; cost: number; count: number }> = {};
    periodUsage.forEach(usage => {
      ...
      qualityUsage[usage.passQuality].cost += usage.consumables.reduce((sum, c) => sum + c.cost, 0);
