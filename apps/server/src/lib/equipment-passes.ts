...
export class EquipmentPassesService {
	...
	static getDefaultPassConfigurations(): Record<string, PassConfiguration> {
		return {
			...
			normal: {
				...
				description:
					"Qualidade normal - equilibrio entre velocidade e qualidade",
			},
			high: {
				quality: "high",
				speed: 30, // m²/h
			...
			photo: {
				...
				description:
					"Qualidade fotográfica - máxima qualidade, menor velocidade",
			},
		};
	}
	...
	static calculatePrintJob(calculation: PrintJobCalculation): PrintJobResult {
		...
		const energyCost =
			timeRequired *
			equipmentConfig.energyCostPerHour *
			passConfig.powerConsumption;
		...
		const printHeadWearPercent =
			(area / equipmentConfig.printHeadLifespan) *
			passConfig.printHeadWear *
			100;
		const printHeadWearCost =
			(area / equipmentConfig.printHeadLifespan) *
			passConfig.printHeadWear *
	...
	static comparePassOptions(
		...
		const passConfigs = EquipmentPassesService.getDefaultPassConfigurations();
		const results: Record<string, PrintJobResult> = {};

		for (const [quality, passConfig] of Object.entries(passConfigs)) {
			results[quality] = EquipmentPassesService.calculatePrintJob({
				area,
				passConfig,
				equipmentConfig,
			});
	...
	static suggestBestPass(
		...
		recommended: string;
		reason: string;
		options: Record<string, PrintJobResult>;
	} {
		const options = EquipmentPassesService.comparePassOptions(
			area,
			equipmentConfig,
		);
		...
		const filteredOptions = Object.entries(options).filter(
			([quality, result]) => {
				if (criteria.maxBudget && result.costs.total > criteria.maxBudget)
					return false;
				if (criteria.maxTime && result.timeRequired > criteria.maxTime)
					return false;
		...
		if (filteredOptions.length === 0) {
			return {
				recommended: "normal",
				reason:
					"Nenhuma opção atende aos critérios especificados. Usando qualidade normal.",
				options,
			};
		}
		...
		switch (criteria.priority) {
			...
			case "quality": {
				const qualityOrder = ["draft", "normal", "high", "photo"];
				const highestQuality = filteredOptions.reduce(
					(max, [quality, result]) => {
						const currentIndex = qualityOrder.indexOf(quality as any);
			...
			case "balanced": {
				...
				const scored = filteredOptions.map(([quality, result]) => {
					...
					const totalScore =
						costScore * 0.4 + speedScore * 0.3 + qualityScore * 0.3;
	...
	static calculatePrintHeadLifespan(
		...
		const totalRecentArea = recentUsage.reduce(
			(sum, usage) => sum + usage.area,
			0,
		);
		const wearRate = totalRecentArea / 30; // m² por dia

		const estimatedDaysRemaining =
			wearRate > 0 ? remainingLifespan / wearRate : Number.POSITIVE_INFINITY;
