...
export class EquipmentDepreciationService {
	...
	static calculateDepreciation(config: DepreciationConfig): DepreciationResult {
		...
		if (method === "linear") {
			...
			if (usefulLifeHours) {
				...
				accumulatedDepreciation =
					EquipmentDepreciationService.calculateAccumulatedAccelerated(
						depreciableValue,
						usefulLifeHours,
						accumulatedHours,
					);
		...
		const result: DepreciationResult = {
			...
			remainingValue,
			remainingLife: {
				hours: usefulLifeHours
					? Math.max(0, usefulLifeHours - accumulatedHours)
					: undefined,
				years: usefulLifeYears
					? Math.max(
							0,
							usefulLifeYears -
								(accumulatedHours ? accumulatedHours / 2000 : 0),
						)
					: undefined,
			},
			needsReplacement,
	...
	static calculateDepreciationCostPerHour(config: DepreciationConfig): number {
		const depreciation =
			EquipmentDepreciationService.calculateDepreciation(config);
	...
	static needsRenewal(config: DepreciationConfig): {
		...
		const remainingLifeRatio =
			depreciation.remainingValue / config.acquisitionValue;
		...
		return {
			...
			timeRemaining: {
				...
				months: depreciation.remainingLife.years
					? depreciation.remainingLife.years * 12
					: undefined,
			},
		};
	}
	...
	static generateDepreciationReport(config: DepreciationConfig) {
		const depreciation =
			EquipmentDepreciationService.calculateDepreciation(config);
		const renewal = EquipmentDepreciationService.needsRenewal(config);
			...
			depreciation: {
				...
				percentage:
					(depreciation.accumulatedDepreciation /
						(config.acquisitionValue - config.residualValue)) *
					100,
			},
			lifespan: {
				...
				usagePercentage: config.usefulLifeHours
					? ((config.accumulatedHours || 0) / config.usefulLifeHours) * 100
					: 0,
			},
			renewal: {
