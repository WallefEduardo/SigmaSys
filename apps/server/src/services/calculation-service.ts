...
import { FormulaEngine, type FormulaVariables, type CalculationResult } from '../lib/formula-engine';
  ...
  async calculateProductCost(
    ...
    try {
      ...
      const materialRules = await this.db.productMaterialRule.findMany({
        ...
        include: {
          material: { select: { id: true, name: true, cost: true, unit: true } },
          calculationRule: { select: { id: true, name: true, formula: true, resultUnit: true } },
      ...
      for (const rule of materialRules) {
        try {
          ...
          const adjustedQuantity = calculation.calculatedQuantity * Number(rule.multiplier) * (1 + Number(rule.wasteFactor));
          ...
          errors.push(`Erro no material ${rule.material.name}: ${(error as Error).message}`);
        }
      }
