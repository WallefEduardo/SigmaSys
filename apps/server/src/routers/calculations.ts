...
export const calculationsRouter = router({
	...
	debugEquipmentCosts: protectedProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			const { EquipmentCostCalculator } = await import(
				"../lib/equipment-cost-calculator"
			);
	...
	calculateProductCost: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			try {
				...
				logger.info(
					`🧮 Processando ${input.checklistItems.materials.length} materiais`,
				);
				for (const material of input.checklistItems.materials) {
					...
					try {
						...
						if (
							...
							try {
								const calculation =
									await calculationService.calculateMaterialCost(
										material.materialId,
										material.calculationRuleId,
										input.measurements,
										companyId,
								...
								logger.error(
									`❌ Erro no CalculationService para material ${material.materialId}:`,
									calculationError,
								);
								...
								if (material.calculationRuleFormula) {
									...
									calculatedQuantity = evaluate(
										material.calculationRuleFormula,
										input.measurements,
									);
									logger.info(
										`🧮 Quantidade calculada manualmente: ${calculatedQuantity}`,
									);
								}
							}
						} else {
						...
						if (!materialData) {
							logger.warn(
								`❌ Material ${material.materialId} não encontrado na base de dados`,
							);
							continue;
						}
						...
						if (totalCost === 0 && calculatedQuantity > 0) {
							totalCost =
								unitCost * calculatedQuantity * (material.multiplier || 1);
							logger.info(
								`🧮 Cálculo manual: ${unitCost} x ${calculatedQuantity} x ${material.multiplier || 1} = R$ ${totalCost}`,
							);
						}
						...
						if (calculatedQuantity === 0) {
							logger.warn(
								`⚠️ Material ${materialData.name} com quantidade calculada = 0, verificando se deve ser incluído`,
							);
						}

						if (totalCost === 0) {
							logger.warn(
								`⚠️ Material ${materialData.name} com custo total = 0, verificando se deve ser incluído`,
							);
						}
						...
						logger.info(
							`✅ Material ${materialData.name} adicionado: R$ ${totalCost.toFixed(2)}`,
						);
					} catch (error) {
						logger.error(
							`❌ Erro ao calcular custo do material ${material.materialId}:`,
							error,
						);
					}
				}
				...
				logger.info(
					`📊 Subtotal de materiais: R$ ${breakdown.materials.subtotal.toFixed(2)} (${breakdown.materials.items.length} itens)`,
				);

				// 2. CALCULAR CUSTOS DOS EQUIPAMENTOS COM CONSUMÍVEIS
				logger.info(
					`🛠️ Processando ${input.checklistItems.equipments.length} equipamentos`,
				);
				for (const equipment of input.checklistItems.equipments) {
					...
					try {
						...
						if (!equipmentData) {
							logger.warn(
								`❌ Equipamento ${equipment.equipmentId} não encontrado na base de dados`,
							);
							continue;
						}
						...
						if (areaProcessed > 0) {
							try {
								...
								const costData =
									await costCalculator.getCostForProductCalculation(
										equipment.equipmentId,
										equipmentData.defaultPassKey || undefined,
									);

								logger.info(
									"💰 Dados de custo obtidos do EquipmentCostCalculator:",
								);
								...
								logger.info(
									`   - inkCostPerM2: ${costData?.passCost?.inkCostPerM2}`,
								);
								logger.info(
									`   - headCostPerM2: ${costData?.passCost?.headCostPerM2}`,
								);
								logger.info(
								...
								if (costData.passCost) {
									...
									totalCost =
										costPerM2 * areaProcessed * (equipment.multiplier || 1);

									// Buscar breakdown detalhado dos custos fixos
									const breakdown = await costCalculator.getOrganizedCosts(
										equipment.equipmentId,
									);

									// Verificar nomes corretos dos campos
									const inkCostPerM2 =
										costData.passCost.inkCostPerM2 ||
										costData.passCost.totalInkCostPerM2 ||
										0;
									const headCostPerM2 =
										costData.passCost.headCostPerM2 ||
										costData.passCost.totalHeadCostPerM2 ||
										0;
									...
									costBreakdownDetail = {
										depreciation:
											Number(breakdown.depreciationPerM2 || 0) * areaProcessed,
										energy: Number(breakdown.energyPerM2 || 0) * areaProcessed,
										maintenance:
											Number(breakdown.maintenancePerM2 || 0) * areaProcessed,
									...
									logger.info(
										`   - printHeadCostsTotal: ${printHeadCostsTotal}`,
									);

									logger.info(
										`✅ Custo completo calculado: R$ ${costPerM2}/m² x ${areaProcessed}m² = R$ ${totalCost.toFixed(2)}`,
									);
									logger.info(
										`🎯 Breakdown: Tinta R$ ${costBreakdownDetail.inkCosts.toFixed(2)} + Cabeça R$ ${costBreakdownDetail.printHeadCosts.toFixed(2)} + Fixo R$ ${costBreakdownDetail.depreciation.toFixed(2)}`,
									);
								} else {
									// Fallback para custo fixo apenas
									logger.warn(
										"⚠️ Nenhum custo variável encontrado (passCost = null/undefined)",
									);
									...
									logger.warn(
										`⚠️ Usando apenas custos fixos: R$ ${costPerM2}/m² x ${areaProcessed}m² = R$ ${totalCost.toFixed(2)}`,
									);
								}
							} catch (costCalculatorError) {
								logger.warn(
									"⚠️ Erro no calculador avançado, usando custo básico:",
									costCalculatorError,
								);
								// Fallback para custo básico
								const costPerM2 =
									Number(equipmentData.calculatedCostPerM2) || 0;
								totalCost =
									costPerM2 * areaProcessed * (equipment.multiplier || 1);
						...
						logger.info(
							`   - depreciation: ${costBreakdownDetail?.depreciation}`,
						);
						logger.info(`   - energy: ${costBreakdownDetail?.energy}`);
						logger.info(
							`   - maintenance: ${costBreakdownDetail?.maintenance}`,
						);
						logger.info(`   - inkCosts: ${costBreakdownDetail?.inkCosts}`);
						logger.info(
							`   - printHeadCosts: ${costBreakdownDetail?.printHeadCosts}`,
						);
						...
						logger.info(
							`✅ Equipamento ${equipmentData.name} adicionado: R$ ${totalCost.toFixed(2)}`,
						);
					} catch (error) {
						logger.error(
							`❌ Erro ao calcular custo do equipamento ${equipment.equipmentId}:`,
							error,
						);
					}
				}
				...
				logger.info(
					`🛠️ Subtotal de equipamentos: R$ ${breakdown.equipments.subtotal.toFixed(2)} (${breakdown.equipments.items.length} itens)`,
				);

				// 3. CALCULAR CUSTOS DOS PROCESSOS
				logger.info(
					`⚙️ Processando ${input.checklistItems.processes.length} processos`,
				);
				for (const process of input.checklistItems.processes) {
					...
					try {
						...
						if (!processData) {
							logger.warn(
								`❌ Processo ${process.processId} não encontrado na base de dados`,
							);
							continue;
						}
						...
						const totalCost =
							costPerHour * timeRequired * (process.multiplier || 1);
						logger.info(
							`💰 Custo do processo: R$ ${costPerHour}/h x ${timeRequired}h x ${process.multiplier || 1} = R$ ${totalCost.toFixed(2)}`,
						);
						...
						logger.info(
							`✅ Processo ${processData.name} adicionado: R$ ${totalCost.toFixed(2)}`,
						);
					} catch (error) {
						logger.error(
							`❌ Erro ao calcular custo do processo ${process.processId}:`,
							error,
						);
					}
				}
				...
				logger.info(
					`⚙️ Subtotal de processos: R$ ${breakdown.processes.subtotal.toFixed(2)} (${breakdown.processes.items.length} itens)`,
				);
				...
				logger.info(
					`✅ Cálculo concluído em ${Date.now() - startTime}ms. Total: R$ ${breakdown.grandTotal.toFixed(2)}`,
				);
					...
					latestCostBreakdown: e.costBreakdowns[0]
						? {
								depreciationPerM2: Number(
									e.costBreakdowns[0].depreciationPerM2,
								),
