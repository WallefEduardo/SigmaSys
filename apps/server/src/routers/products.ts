  ...
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const product = await ctx.db.$transaction(async (tx) => {
        const { materials, equipments, processes, finishes, ...productData } = input
    ...
    .query(async ({ ctx, input }) => {
      ...
      const product = await ctx.db.product.findFirst({
        ...
        include: {
          ...
          equipments: {
            include: {
              equipment: { select: { name: true, costPerHour: true, type: true } }
            }
          },
          processes: {
            include: {
              process: { select: { name: true, costPerHour: true, timeUnit: true } }
            }
          },
          finishes: {
            include: {
      ...
      for (const productEquipment of product.equipments) {
        ...
        const equipmentCost = Number(productEquipment.equipment.costPerHour) * timeNeeded
      ...
      for (const productProcess of product.processes) {
        ...
        const processCost = Number(productProcess.process.costPerHour) * timeNeeded
      ...
      for (const productFinish of product.finishes) {
        const finishCost = Number(productFinish.finish.cost) * Number(productFinish.quantity)
      ...
      return {
        ...
        marginPercent: totalCost > 0 ? ((finalPrice - totalCost) / totalCost) * 100 : 0,
        breakdown
      }
    }),
  ...
  stats: protectedProcedure
    ...
    .query(async ({ ctx }) => {
      ...
      return {
        ...
        productsByCategory: productsByCategory.map(p => ({ category: p.category, count: p._count })),
        recentProducts
      }
    })
})