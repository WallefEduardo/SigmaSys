...
export const finishesRouter = router({
	...
	create: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (input.composition.materials) {
				const materialIds = input.composition.materials.map(
					(m) => m.materialId,
				);
		...
		.query(async ({ ctx, input }) => {
			...
			if (composition?.materials) {
				...
				for (const materialComp of composition.materials) {
					const material = materials.find(
						(m) => m.id === materialComp.materialId,
					);
					if (material) {
						const materialCost =
							Number(material.cost) * materialComp.quantity * input.quantity;
			...
			if (composition?.processes) {
				...
				for (const processComp of composition.processes) {
					...
					if (process) {
						const processCost =
							Number(process.costPerHour) *
							processComp.timeNeeded *
							input.quantity;
	...
	stats: protectedProcedure.query(async ({ ctx }) => {
		...
		const [totalFinishes, activeFinishes, avgCost, recentFinishes] =
			await Promise.all([
